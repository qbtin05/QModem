...
QModem 现已采用纯 JS Luci 前端（`luci-app-qmodem-next`），不再依赖 luci-compat，减少了 Luci 21 及之后版本的兼容性问题。

### 主要功能：
1. 优化了拨号配置界面，重新设计了拨号日志和拨号状态的显示逻辑。
2. 优化了短信功能，整合到主界面中。短信功能以对话框形式呈现，自动导出到路由的文件系统，支持记录已发信息。
3. 优化了AT调试、模组高级功能的界面。
4. 优化了所有设置界面。
5. 全面的翻译支持，翻译覆盖率大幅提升。
6. 短信转发：已读短信也会转发。
...
### 重要说明：
1. MWAN、TTL 功能在此版本中已被移除。
2. 短信后端采用 JSON 作为数据库以简化实现。
3. 短信管理：不支持在 UI 设置短信储存位置，不支持在 UI 删除短信。仅可选择自动删除短信。
4. 已知问题：短信拼接基于 timestamp+reference ID，但是由于部分运营商和模组提供的 timestamp 并不可靠，导致长信息无法拼接。
...
要安装QModem，请将自定义Feed源添加到您的OpenWRT编译环境，并在 `make menuconfig` 中选择 `luci-app-qmodem-next` 软件包。

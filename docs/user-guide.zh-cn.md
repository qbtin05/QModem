...
* **`luci-app-qmodem-next` (核心)**: 提供模组管理、状态监控、拨号和集成短信功能的核心 Web 界面的主软件包。此软件包使用纯 JS LuCI 前端，消除了对 luci-compat 的依赖。
* **`luci-app-qmodem-hc`**: 支持具有硬件控制 SIM 卡插槽的特定设备（例如 HC-G80），允许从 UI 切换 SIM 卡。
* **`luci-app-qmodem-monitor`**: 为模组状态提供额外的监控功能。

**注意**：旧版软件包（`luci-app-qmodem`、`luci-app-qmodem-sms`、`luci-app-qmodem-mwan`、`luci-app-qmodem-ttl`）已被移除。MWAN 和 TTL 功能不再可用。
...
### `luci-app-qmodem-next` (核心)

主插件为管理您的模组提供了全面的界面，采用现代纯 JS LuCI 前端。

* **模组信息**: 显示详细状态，包括制造商、型号、固件、IMEI、信号质量 (RSSI, RSRP, RSRQ, SINR) 和网络注册详情。
  ![模组信息](../imgs/modem_info.png)
* **拨号控制**: 允许您配置和控制模组的数据连接，重新设计了拨号日志和拨号状态的显示逻辑。
  ![拨号总览](../imgs/dial_overview.png)
* **集成短信功能**: 完整的短信功能以对话框形式呈现，支持发送和接收短信，自动导出到路由的文件系统，并记录已发信息。
* **高级调试**: 提供锁频段、锁小区和发送自定义 AT 命令的工具。
  ![高级设置](../imgs/modem_debug_lock_band.png)

### `luci-app-qmodem-hc`

为使用 GPIO 控制活动 SIM 卡插槽的特定硬件型号（例如 HC-G80）提供一个用于在两个 SIM 卡之间切换的 UI。
